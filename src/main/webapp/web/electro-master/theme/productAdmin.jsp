    ...
    <c:forEach items="${products}" var="b">
      <tr>
        <c:if test="${b.getStatus() == 1}">
          ...
          <td><img src="${b.getImg()}" style="height: 100px ; width: 120px" alt=""></td>
          <td>${b.getAmount()}</td>
          <td>${b.getCategory().getName()}</td>
          <td>${b.getDiscount()}</td>
          <td>${b.getBrand().getName()}</td>
        </c:if>
        <td><button><a style="text-decoration: none" href="/productAdmin?action=delete&id=${b.getId()}">Delete</a></button></td>
      </tr>
    </c:forEach>
  </table>
</div>
